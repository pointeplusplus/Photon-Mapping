...
Vec3f mixColors(std::vector<double> wavelengths){
	...
	for(int i = 0; i < wavelengths.size(); i++){
		Vec3f xyz = RGBtoXYZ(wavelengthToRGB(wavelengths[i]));
		Y += xyz.y();
	}
	for(int i = 0; i < wavelengths.size(); i++){
		Vec3f xyz = RGBtoXYZ(wavelengthToRGB(wavelengths[i]));
	...
	Y = Y / wavelengths.size();
	x = x / wavelengths.size();
	y = y / wavelengths.size();
	double X = x / y * Y;
	double Z = (1 - x - y) / y * Y;
...
int main() {
	...
	for(double lambda = 380; lambda < 780; lambda++){
